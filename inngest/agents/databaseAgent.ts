...
const saveToDatabaseTool = createTool({
  ...
  parameters: z.object({
    ...
    userId: z.string().describe("The ID of the user who owns the receipt"), // this is needed for authentication
    ...
    receiptSummary: z
      .string()
      .describe(
        "A summary of the receipt, including the merchant name, address, contact, transactoin date, transaction amount, and currency. Include a human readable summary of the receipt. Mention both invoice number and receipt number if both are present. Include some key details about the items on the receipt, this is a special featured summary so it should include some key details about the items on the receipt with some context.",
      ),
    currency: z.string(),
    items: z.array(
      z
  ...
  handler: async (params, context) => {
    const {
      fileDisplayName,
      receiptId,
      userId, // this is needed for authentication
      merchantName,
      merchantAddress,
      merchantContact,
      transactionDate,
    ...
    const result = await context.step?.run(
      ...
      async () => {
        try {
          ...
          const { userId: outputUserId } = await convex.mutation(
            api.receipts.updateReceiptWithExtractedData,
            {
              receiptId: receiptId as Id<"receipts">,
              userId, // this is needed for authentication
              secret: process.env.INNGEST_SECRET!, // pass the secret for authentication
              fileDisplayName,
              merchantName,
              merchantAddress,
              merchantContact,
    ...
    if (result?.addedToDb === "Success") {
      ...
      context.network?.state.kv.set("receipt", true);
    }
